package net.rexbr.preludejurassika.entity.reorganized.proterosuchus;
...
public class ProterosuchusEntity extends Animal implements IAnimatable {
    private AnimationFactory factory = new AnimationFactory(this);

    public ProterosuchusEntity(EntityType<? extends Animal> entityType, Level level) {
    ...
    private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
        if (event.isMoving()) {
            event.getController().setAnimation(new AnimationBuilder().addRepeatingAnimation("animation.proterosuchus.walk", 999));
            return PlayState.CONTINUE;
        }



        event.getController().setAnimation(new AnimationBuilder().addRepeatingAnimation("animation.proterosuchus.idle", 999));
        return PlayState.CONTINUE;
    }
