package net.rexbr.preludejurassika.entity.common.deinonychus;
...
public class DeinonychusEntity extends Animal implements IAnimatable {
    private AnimationFactory factory = new AnimationFactory(this);

    public DeinonychusEntity(EntityType<? extends Animal> entityType, Level level) {
    ...
    private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
        if (event.isMoving()) {
            event.getController().setAnimation(new AnimationBuilder().addRepeatingAnimation("animation.deinonychus.walk", 999));
            return PlayState.CONTINUE;
        }



        event.getController().setAnimation(new AnimationBuilder().addRepeatingAnimation("animation.deinonychus.idle", 999));
        return PlayState.CONTINUE;
    }
    ...
    private PlayState attackPredicate(AnimationEvent event) {
        if(this.swinging && event.getController().getAnimationState().equals(AnimationState.Stopped)) {
            ...
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.deinonychus.attack", false));
            this.swinging = false;
        }
