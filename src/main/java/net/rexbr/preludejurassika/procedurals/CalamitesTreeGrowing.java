...
public class CalamitesTreeGrowing {
    ...
    public static void execute(LevelAccessor world, double x, double y, double z) {
        class TstWait5 {
            ...
            private void run() {
                ...
                if ((world.getBlockState(new BlockPos(x, y, z))).getBlock() == ModBlocks.CALAMITES_SAPLING2.get()) {
                    if (Math.random() > 0.40) {
                        if (world instanceof ServerLevel _serverworld) {
                            StructureTemplate template = _serverworld.getStructureManager().getOrCreate(new ResourceLocation("prelude", "calamites"));
                            if (template != null) {
                                template.placeInWorld(_serverworld, new BlockPos(x - 6, y, z - 6), new BlockPos(x - 1, y, z - 1),
                        ...
                        if (world instanceof ServerLevel _serverworld) {
                            StructureTemplate template = _serverworld.getStructureManager().getOrCreate(new ResourceLocation("prelude", "calamites2"));
                            if (template != null) {
        ...
        new TstWait5().start(world, Mth.nextInt(new Random(), 100, 200));
    }
